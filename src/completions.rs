...
pub fn completions(shell: Option<&str>) -> Result<()> {
    match shell {
        Some("bash") => generate(Bash, &mut build_cli(), "flavours", &mut io::stdout()),
        Some("elvish") => generate(Elvish, &mut build_cli(), "flavours", &mut io::stdout()),
        Some("powershell") => {
            generate(PowerShell, &mut build_cli(), "flavours", &mut io::stdout())
        }
        Some("fish") => {
            let mut buffer = Vec::new();
            generate(Fish, &mut build_cli(), "flavours", &mut buffer);
        ...
        Some("zsh") => {
            let mut buffer = Vec::new();
            generate(Zsh, &mut build_cli(), "flavours", &mut buffer);
            let buffer = String::from_utf8(buffer)?;
            let buffer = buffer.replace(
